...
遗传算法睡一种解决最优化的搜索算法。以下的遗传算法能用重现画像。编程都在GitHub上传，有兴趣去看看: <a href="https://github.com/kennycason/genetic_draw" target="blank">here</a>

### 算法概念

这编程中的遗传算法有两种。各算法都是用遗传算法把随机的画像进化成目标的画像。目标画像称为目标。

个体的遗传是基因序列，各基因算是一个形状造成。可能性的形状包含圆，椭圆，三角形，四角形，矩形，多边形，像素。除了形状以外基因还包容别的信息。各基因包含形状的颜色，透明度，位置（x,y,z轴)，大小。

基因表达过程很直接，个体基因中的形状画在黑色的画像上（根据z轴顺序）。因为这应用的进化从完全随机个体的种群开始，所以第一代表达的基因的结果平均跟目标画像完全不像。基因表达后，通过目标画像和表达画像比较我们来算个体的适应度。这个比较函数是一种适应函数。在这个编程我们用的适应函数是算目标画像和表达画像的各像素的误差，把各误差都加起来算个体的总体误差。这个适应度会来决定谁生存谁死，也叫适者生存。在很多应用高的适应度更好，但是在这个应用因为我们在算错误（错误函数），所以完美的适应度是0。0的适应度表达进化的个体表达的画像跟目标画像一摸一样。用更正式的写法：错误函数 ＝ 最大适应度 － 错误。

#### 算法参数

1. 种群规模：模型个体的数目。
2. 基因串长度：个体中染色体的数目。
3. 交叉概率：两个个体（A和B）繁殖的时候，孩子从A个体收到基因的概率。收到B个体基因的概率是1.0 － 收到A个体基因的概率。
...
但是, 看一下进化的Kirby。他明显有点奇怪。形状不错，但是颜色不对。这是用不好适应函数的结果。我原来开发的适应函数评价各画像的像素的时候发生了一个小问题。各像素是24bit，高位8bit是赤，中位8bit是绿，低位8bit是青。想起我们以上谈的误差函数，一个像素的误差 ＝ A画像的像素 － B画像的像素。根据RGB（赤，绿，青）的位，因为赤的位最高，所以赤的误差比别的位更影响个体的适应。互相，因为青的位最低，随意青的误差对个体的适应度比较少。

<img src="/images/genetic_draw/kirby_evolved_bad_fitness_function.png" width="128px"/>

更多统计和图片可以下载看看（XLSX）[here](https://github.com/kennycason/genetic_draw/raw/master/convergence_stats.xlsx).
