...
// ---------------------------------------------------------------------------------
// ---------------------------------------------------------------------------------
int SDLMappy::MapDecodeLYR (int layer)
{
	mapmappt[layer] = (short int *) malloc (Mapbyteswapl(mapgenheader.headsize));
	if (mapmappt[layer]==NULL) { maperror = MER_OUTOFMEM; return -1; }
	fread (mapmappt[layer], Mapbyteswapl(mapgenheader.headsize), 1, mapfilept);
	return 0;
}
...
// ---------------------------------------------------------------------------------
// change le numero de la layer (0 � 7) devant etre affichee
// ---------------------------------------------------------------------------------
...
int SDLMappy::MapDrawBGT (SDL_Surface * lpDDS )
{
	...
	for(i=0; i<lpDDS->h/mapblockheight+2;i++)
		for(j=0; j<lpDDS->w/mapblockwidth+2;j++)
		{
			...
			TileDestRect.h	= mapblockheight;
			TileDestRect.w	= mapblockwidth;
                   //     printf("TILE DEST:\nX %d\nY %d\n",TileDestRect.x,TileDestRect.y);
                 //  if(TileDestRect.x >= -mapblockwidth && TileDestRect.y >= -mapblockwidth) {
                     //   printf("Before set numtile\n");
			numtile = mappt[mapxo+j+((mapyo+i)*mapwidth)];
                     //   printf("After set numtile: %d\n",numtile);
			if (((mapxo+j)<mapwidth) && ((mapyo+i)<mapheight) && ((mapxo+j)>=-1) && ((mapyo+i)>=-1)) {
			    if (numtile >= 0) {
                        //        printf("Blitting tile! %d\n",numtile);
					SDL_BlitSurface(maplpDDSTiles[numtile>>5],
										NULL, lpDDS, &TileDestRect);
                            } else {/*
                                printf("tilenum less than 0 begin\n");
					anim = (ANISTR *) (mapanimstrendpt + numtile);
					...
					SDL_BlitSurface(maplpDDSTiles[numanim],
										NULL, lpDDS, &TileDestRect);
                                        printf("tilenum less than 0 end\n");*/
				}
			}
                 //  }
		}
...
int SDLMappy::DrawParallax (SDL_Surface * lpDDS )
{
	...
	for(i=-1; i<lpDDS->h/PARAH+2;i++)
		for(j=-1; j<lpDDS->w/PARAW+2;j++)
		{
...
SDL_Surface* SDLMappy::Get(int x, int y) {
    return NULL;
}
